...
class Product_categories extends MY_Controller
{
    ...
    public function index()
    {
         ...
         */
        $config = [];
        $config['base_url'] = base_url(lang("routes_products") . "/");
        ...
        $config['total_rows'] = $this->general_model->rowCount("product_categories sc", $wheres, $likes, $joins, [], $distinct, $groupBy, "sc.id");
        $config['per_page'] = 12;
        $config["num_links"] = 5;
         ...
         * Get Products
         */
        $this->viewData->product_categories = $this->general_model->get_all("product_categories sc", $select, $order, $wheres, $likes, $joins, [$config["per_page"], $offset], [], $distinct, $groupBy);
        /**
         * Meta
         */
        $this->viewData->page_title = (!empty($category) ? strto("lower|ucwords", $category->title) : strto("lower|ucwords", lang("product_categories")));
